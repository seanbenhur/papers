...
class ResBlock(nn.Module):
    ...
    def __init__(self, in_channels, int_channels, identity_downsample=None, stride=1):
        ...
        self.conv1 = nn.Conv2d(
            in_channels, int_channels, kernel_size=1, stride=1, padding=0
        )
        self.bn1 = nn.BatchNorm2d(int_channels)
        self.conv2 = nn.Conv2d(
            int_channels, int_channels, kernel_size=3, stride=stride, padding=1
        )
        self.bn2 = nn.BatchNorm2d(int_channels)
        self.conv3 = nn.Conv2d(
            int_channels,
            int_channels * self.expansion,
            kernel_size=1,
            stride=1,
            padding=0,
        )
    ...
    def _make_layer(self, block, num_res_blocks, int_channels, stride):
        ...
        if stride != 1 or self.in_channels != int_channels * 4:
            identity_downsample = nn.Sequential(
                nn.Conv2d(
                    self.in_channels, int_channels * 4, kernel_size=1, stride=stride
                ),
                nn.BatchNorm2d(int_channels * 4),
            )
            layers.append(
                ResBlock(self.in_channels, int_channels, identity_downsample, stride)
            )
