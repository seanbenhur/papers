...
class QRNN(nn.Module):
  def __init__(self,vocab_size,embed_size,n_filters,
               kernel_size,batch_size,seq_len,n_layers,device,dropout):
    super().__init__()
    self.embed_size = embed_size
    self.n_filters = n_filters
    self.kernel_size = kernel_size
    ...
    self.qrnn_layers = nn.ModuleList([QRNNLayer(self.batch_size, embed_size if self.n_layers==0 else self.n_filters,
                                                self.n_filters, self.kernel_size,self.embed_size,self.device,dropout) for i in range(self.n_layers)])
